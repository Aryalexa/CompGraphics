...
void Camara::resize(int newWidth, int newHeight){
	WIDTH= newWidth;
	HEIGHT= newHeight;
	GLdouble RatioViewPort= (float)WIDTH/(float)HEIGHT;
	glViewport (0, 0, WIDTH, HEIGHT) ;

	GLdouble SVAWidth= right-left;
	GLdouble SVAHeight= top-bottom;
	GLdouble SVARatio= SVAWidth/SVAHeight;
	if (SVARatio >= RatioViewPort) {
		GLdouble newHeight= SVAWidth/RatioViewPort;
		GLdouble yMiddle= ( bottom+top )/2.0;
		top= yMiddle + newHeight/2.0;
		bottom= yMiddle - newHeight/2.0;
    }
	else {
		GLdouble newWidth= SVAHeight*RatioViewPort;
		GLdouble xMiddle= ( left+right )/2.0;
		right= xMiddle + newWidth/2.0;
		left=  xMiddle - newWidth/2.0;
	}

	//TODO, cambiar a cam->protyecta??
	this->setVolumenVista();
...
void Camara::cambiaPos(PV3D *i_eye, PV3D *i_look){

	GLdouble eyeX = eye->getX() + i_eye->getX();
	GLdouble eyeY = eye->getY() + i_eye->getY();
	GLdouble eyeZ = eye->getZ() + i_eye->getZ();
	eye = new PV3D(eyeX, eyeY, eyeZ, 1);

	GLdouble lookX = i_look->getX();//look->getX() + i_look->getX();//
	GLdouble lookY = i_look->getY();//look->getY() + i_look->getY();//
	GLdouble lookZ = i_look->getZ();//look->getZ() + i_look->getZ();//
