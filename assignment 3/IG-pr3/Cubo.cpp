...
Cubo::Cubo()
{
	...
	GLfloat half = 1.0/2;
	numVertices = 8;
	PV3D** vert = new PV3D*[8]; //cara de abajo en orden + cara de arriba en orden
	vert[0] = new PV3D(-half, -half, -half, 1);
	vert[1] = new PV3D(half, -half, -half, 1);
	vert[2] = new PV3D(half, -half, half, 1);
	vert[3] = new PV3D(-half, -half, half, 1);

	vert[4] = new PV3D(-half, half, -half, 1); // 0
	vert[5] = new PV3D(-half, half, half, 1); // 3
	vert[6] = new PV3D(half, half, half, 1); // 2
	vert[7] = new PV3D(half, half, -half, 1); // 1
